...
import { createDutyReportFromAnalysis } from '@/services/dutyReportService';
    ...
    setTimeout(async () => {
      try {
        ...
        const mockAnalysis = {
          types: ['지시사항', '순찰', '인수인계'],
          meeting: {
            datetime: "2024-06-26, 당직사령관 지시",
            reports: "각 시도 재난 정보 확인, 산불대비 상황 점검",
            abnormalities: "특이사항 없음",
            handover: "다음 당직자에게 당일 지시사항 및 순찰 결과 전달"
          },
          inspection: {
            datetime: "2024-06-26 17:00-18:00",
            content: "정기 업무 순찰 실시",
            actions: "시설 전반 점검 완료, 보안 상태 확인",
            notes: "산불대비 관련 지시사항에 따른 추가 점검 실시"
          },
          handover: {
            issues: handoverSummary.issues > 0 ? `${handoverSummary.issues}건의 미처리 사항` : "정기 점검 완료, 특이사항 없음, 업무 처리 정상",
        ...
        // 당직 보고서에 저장
        await createDutyReportFromAnalysis(mockAnalysis, inputContent);
        
        setAnalyzedData(mockAnalysis);
        setIsAnalyzing(false);
        toast.success("분석이 완료되었습니다. 인수인계 현황이 업데이트되고 당직 보고서에 저장되었습니다.");
      } catch (error) {
        console.error('Error saving to duty report:', error);
        setIsAnalyzing(false);
        toast.error("분석은 완료되었지만 당직 보고서 저장 중 오류가 발생했습니다.");
